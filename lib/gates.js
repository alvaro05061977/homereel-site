...
import { rerunRoom } from "./rerun.js";
...
export async function afterVerdict(orderPhotoRecordId, gate, token, base) {
  ...
  try {
    ...
    // ---- was THIS verdict a Re-run? ------------------------------------
    //
    // Then that one room goes straight back through its own node — a keyframe
    // redraw is 100 credits and the clip it prevents is 3,950, so there is no
    // reason to make a human come back and press anything.
    //
    // Checked before the gate count, because a re-run blanks the verdict it
    // just read, and the count below must be taken AFTER that has happened.
    const thisVerdict = nameOf(photo.fields?.[gate === "clip" ? F.clipVerdict : F.keyframeVerdict]);
    if (thisVerdict === "Re-run") {
      try {
        out.rerun = await rerunRoom(orderPhotoRecordId, gate, token);
      } catch (e) {
        // Same rule as everything else here: the verdict is saved and that is
        // what matters. A re-run that did not start can be started again.
        console.error("[gates] re-run failed (verdict was still saved):", String(e));
        out.rerun = { ok: false, error: String(e.message || e) };
      }
      // A re-run always leaves the gate incomplete — that room is now Pending
      // again — so there is nothing further to decide.
      return { ...out, checked: true, orderId, complete: false, next: "re-run" };
    }
