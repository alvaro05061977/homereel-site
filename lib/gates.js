...
// Fire the next stage over HTTP, the same way api/stripe-webhook.js starts the
// plates step: a separate endpoint runs in its own invocation with its own
// clock, so the reviewer's browser is never held open by a render that takes
// minutes. We wait a moment to be sure the request landed, then let go.
async function trigger(path, body) {
  const key = process.env.QC_KEY;
  const host = process.env.VERCEL_URL;
  if (!key || !host) {
    return { triggered: false, reason: "QC_KEY or VERCEL_URL missing" };
  }
 ...
 */
export async function afterVerdict(orderPhotoRecordId, gate, token) {
  ...
  try {
    ...
    if (gate === "keyframe") {
      ...
      const fired = await trigger("/api/produce", { order: orderId, stage: "clips" });
