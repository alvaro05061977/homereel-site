...
async function accessToken() {
  ...
  const body = new URLSearchParams({
    ...
    // REQUIRED (learned the hard way, 2026-08-12). Without mcp:custom-audience
    // the auth server happily mints a token whose audience is NOT the MCP
    // resource, and mcp.magnific.com answers every call with a bare
    // 401 {"message":"Unauthenticated."} that looks like a bad credential.
    // The scope is advertised in the protected-resource metadata; the name is
    // the clue. It must ALSO be present on the original device-flow sign-in,
    // because scope is fixed when the refresh token is minted.
    // The EXACT scope set mcp.magnific.com demands. Read it from the horse's
    // mouth if this ever breaks: an unauthenticated POST to the MCP endpoint
    // returns WWW-Authenticate with scope="openid profile email
    // mcp:custom-audience". Dropping profile/email leaves the app unable to
    // resolve which Magnific user the token belongs to, and it answers
    // {"message":"Unauthenticated."} even though the token is valid and
    // correctly addressed. offline_access is ours, for the refresh token.
    ...
    // Together with the scope above this is what stamps aud=https://mcp.magnific.com
    // on the access token. Verified 2026-08-13: without BOTH, the token comes
    // back with aud=null and every MCP call 401s.
    resource: MCP_URL,
  });
  ...
  // by refreshing twice with the same token. Downgraded from a warning to an
  // observation so it stops reading like an outage.
  if (json.refresh_token && json.refresh_token !== refreshToken) {
    console.log("[canvas] note: a new refresh token was issued. Harmless unless the realm also revokes the old one — if calls start failing with invalid_grant, this needs a token store rather than an env var.");
  }

  // Audience/scope of the token we just got, for diagnosing 401s from the MCP
  // server. Claims only — never the token itself.
  try {
    const claims = JSON.parse(Buffer.from(json.access_token.split(".")[1], "base64").toString("utf8"));
    console.log("[canvas] token aud:", JSON.stringify(claims.aud), "scope:", claims.scope);
    if (!claims.aud) {
      console.error("[canvas] TOKEN HAS NO AUDIENCE — mcp.magnific.com will reject it with a bare 401. The OAuth client must be REGISTERED with the mcp:custom-audience scope (requesting it is not enough; Keycloak silently drops a scope the client is not allowed), and both the sign-in and this refresh must send resource=" + MCP_URL);
    }
  } catch {
