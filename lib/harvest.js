 ...
 */
export async function harvestOrder(orderId, airtableToken, { stage, base } = {}) {
  ...
  for (const room of rooms) {
    ...
    if (!run.done) {
      items.push({ label, state: "running", runId, status: run.status });
      continue;
    }
  ...
  for (let i = 0; i < updates.length; i += 10) {
    ...
    try {
      await atJson(encodeURIComponent(ORDER_PHOTOS), airtableToken, {
        ...
        body: JSON.stringify({ records: batch.map(({ id, fields }) => ({ id, fields })), typecast: true }),
      });
