...
export async function startOrder(orderId, airtableToken, { stage = "keyframes", dryRun = false, force = false } = {}) {
  ...
  // ⚠ NO typecast. Production Status is a singleSelect, and with typecast on a
  // misspelled status is silently CREATED as a new option instead of rejected —
  // so the order lands in a state nothing downstream looks for and nothing ever
  // says so. Off, a wrong value fails the write and tells us.
